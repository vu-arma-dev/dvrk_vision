...
class UVToWorldConverter:
    ...
    def toWorldSpace(self,p):
        ...
        normals = np.empty(points2D.shape)
        ...
        for idx, point2D in enumerate(points2D):
            ...
            # pointID = self._cell.GetPointId (0)
            # normals[idx,:] = self.polyData.GetCellData().GetNormals().GetTuple(pointID)
    ...
    def toUVSpace(self,p):
        ...
        for idx, point2D in enumerate(points2D):
            self.pointLocator.FindClosestPoint(point2D, closest, self._cell, cellId, subId, dist)
            self._cell.EvaluatePosition(point2D, closest, subId, pcoords, dist, weights)
            if type(pcoords) == str:
